...
*/
	...
	xTaskCreate( StartTask3, "T3", configMINIMAL_STACK_SIZE, NULL, 2, NULL );
...
static void StartTask3()
{
	...
	for( ;; )
	{
		xSemaphoreTake(xSemaphore, 4000 / portTICK_PERIOD_MS);
		retVal = xSemaphoreTake(xSemaphore, 4000 / portTICK_PERIOD_MS);
		if(retVal == pdTRUE)
		{
			printf("Task3 syncronized!\n");
		}
		else
		{
